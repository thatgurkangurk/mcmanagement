...
pub async fn get_servers() -> Result<Vec<ServerInfo>, ServerFnError> {
    ...
    let context = FullstackContext::current().ok_or_else(|| {
        ServerFnError::new("Server functions must run within an active server scope")
    })?;

    let app_state = context.extension::<Arc<AppState>>().ok_or_else(|| {
        ServerFnError::new("Failed to retrieve AppState from fullstack extension layers")
    })?;
